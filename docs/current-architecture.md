...
Entry- and Author-scope search use the bounded `POST /api/entries/query` and `POST /api/producers/query` contracts, while Tag search keeps its dedicated route. Production builds derive the API root from the current serving origin (`/api/`); development still defaults to `http://127.0.0.1:8765/api/`, with `VITE_API_BASE_URL` remaining the explicit override.
...
`apps/web/src/GalleryApp.vue` consumes this projection, lets the user create an Entry with either a new or existing type, browses Entries by type, and opens composed Entry details.
